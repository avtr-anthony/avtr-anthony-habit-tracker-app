...
export default function CardHabitos({ id, label, description, onDelete }: HabitosCardProps) {
  ...
  const classes = clsx(
    "rounded-lg shadow-lg/5 transition-colors duration-300 ease-in-out w-full max-w-[350px] md:min-w-[350px]  md:min-h-[118px] flex text-right gap-3 overflow-hidden ",
    {
    ...
    <div className="pr-4 pb-4 md:pr-0 md:pb-0">
      <div className={classes}>
        ...
        <div className="flex w-full flex-col justify-center py-4">
          <h3
            className={clsx(
              "text-base font-bold capitalize transition-colors duration-300 ease-in-out",
          ...
          {description && (
            <p className="text-surface line-clamp-1 text-base capitalize">{description}</p>
          )}
        </div>
